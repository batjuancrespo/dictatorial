...
async function initializeFirebase() {
  try {
    ...
    const unsubscribe = db.collection('corrections').onSnapshot(() => {
...
async function loadCorrections(db) {
  ...
  try {
    ...
    if (localCorrections) {
      ...
      console.log('Loaded corrections from local storage:', corrections);
    }

    // If we have a Firebase connection, load from there
    if (db && firestoreConnection) {
      ...
      snapshot.forEach(doc => {
        const data = doc.data();
        if (data.original && data.correction) {
          corrections.set(data.original, data.correction);
          console.log('Loaded correction:', data.original, '->', data.correction);
        }
      });

      // Update local storage
      localStorage.setItem('corrections', JSON.stringify([...corrections]));
      console.log('Updated corrections from Firestore:', corrections);
    }
  } catch (error) {
    console.error('Error loading corrections:', error);
    showSuccessMessage('Error loading corrections, using cached data', {type: 'warning'});
  }
}

async function saveCorrection(original, correction) {
  console.log('Saving correction:', original, '->', correction);
  try {
    // Always update local Map and storage
    ...
    showSuccessMessage('Corrección guardada localmente', {type: 'warning'});
...
function processText(text) {
  ...
  processed = text.toLowerCase().replace(/[.,]/g, '');
...
function applyCorrections(text) {
  if (!text) return text;
  console.log('Applying corrections to text. Current corrections:', corrections);
