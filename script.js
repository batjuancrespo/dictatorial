    ...
    async function initializeRecording() {
      try {
        ...
        mediaRecorder.ondataavailable = handleDataAvailable; // Use the chunking handler
        mediaRecorder.onstop = handleStop; // Use the chunking stop handler
    ...
    async function handleDataAvailable(event) {
      if (event.data.size > 0) {
        ...
        const audioContext = new AudioContext({ sampleRate: 16000 }); // Or your desired sample rate
        ...
        while (currentChunk.length >= maxChunkSize) {
          ...
          sendChunkToAPI(new Int16Array(chunkToSend));
        }
      }
    }
    ...
    async function handleStop() {
      ...
      if (currentChunk.length > 0) {
        sendChunkToAPI(new Int16Array(currentChunk));
      }
      ...
      try {
        const response = await fetch(API_URL, {
          ...
          headers: {
            ...
            'Content-Type': 'application/octet-stream' // Correct content type for Int16Array
          },
          body: int16ArrayChunk.buffer // Send the ArrayBuffer
        });
        ...
        // --- Existing transcription handling logic ---
    ...
    // Text Processing Functions (No changes needed here)
    ...
    // Correction Functions (No changes needed here)
    ...
    // AI Improvement Functions (No changes needed here)
    ...
    // Cleanup Function (No changes needed here)
    function cleanup() {
      if (unsubscribeSnapshot) {
        unsubscribeSnapshot();
      }
    ...
    // Theme Management (No changes needed here)
    ...
    // Keyboard Shortcuts (No changes needed here)
