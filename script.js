...
let sampleRate = 16000; // Default sample rate, will be updated
const maxChunkSize = 30 * sampleRate; // 30 seconds at the determined sample rate
let currentChunk = [];
...
async function initializeRecording() {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({
            audio: {
                sampleRate: sampleRate // Set the sample rate here
            }
        });
...
async function handleDataAvailable(event) {
    ...
    if (event.data.size > 0) {
        ...
        }); // Use the determined sample rate
...
async function sendChunkToAPI(int16ArrayChunk) {
    ...
    try {
        const audioBlob = new Blob([int16ArrayChunk.buffer], {
            type: 'audio/pcm'
        }); // Correct Blob type, use .buffer

        console.log("Blob size:", audioBlob.size, "bytes"); // Log blob size

        const response = await fetch(API_URL, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${API_KEY}`,
                'Content-Type': `audio/pcm; rate=${sampleRate}; channels=1; encoding=signed-integer; sampleFormat=s16` // Explicit Content-Type
            },
        ...
        if (!response.ok) {
            ...
            console.error("Full error response:", errorText); // Log full response
